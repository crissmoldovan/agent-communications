...
| D2 | npm scope | **`@agent-communications`** | The user's npm scope |
...
```
packages/
  comms-core/   @agent-communications/core   provider-neutral library + bin `agentcomms`
  gmail/        @agent-communications/gmail        Gmail library + bin `agent-gmail` (CLI, includes `mcp` subcommand)
  gmail-mcp/    @agent-communications/gmail-mcp    bin `agent-gmail-mcp`: starts the stdio MCP server directly
skills/         gmail-* skills (Agent Skills format), one directory each
```

- **`@agent-communications/core`** — nothing Gmail-specific, but everything email-specific that providers share: the
...
- **`@agent-communications/gmail`** — the Gmail provider and both user-facing surfaces. OAuth (loopback + PKCE, manual
...
- **`@agent-communications/gmail-mcp`** — a thin package whose single bin starts the stdio server
  (`createGmailMcpServer()` from `@agent-communications/gmail`, pinned exact). Exists so client configs can say
  `npx -y @agent-communications/gmail-mcp@X.Y.Z` and so the MCP server has an obvious package name.

"`npx` for each package" means: `npx @agent-communications/core doctor`, `npx @agent-communications/gmail <command>`,
`npx @agent-communications/gmail-mcp`. Each package has exactly one `bin`, which is what `npx` requires **[V]**.

### 4.2 Layering inside `@agent-communications/gmail`
...
- **Shell agents are T2, and the docs say so.** Skills fall back to the CLI, so an agent that uses them has a
  ...
  `Bash(agent-gmail inbox policy*)`, `Bash(agent-gmail send*)` and their `npx * @agent-communications/gmail*` forms, and the pty wrappers (`script`,
...
- `@agent-communications/core`, `@agent-communications/gmail`, `@agent-communications/gmail-mcp`; lockstep versions; ESM;
...
- Every skill works through the MCP tools when connected **or** through the CLI
  (`npx -y @agent-communications/gmail@<version> … --json`) — `npx skills add` installs skills, not servers **[V]**.
...
- `--launcher managed` (default): installs the exact running version into a versioned directory
  ...
  optional keyring (`npm install --prefix … --save-exact @agent-communications/gmail@<version>`, which brings the
  ...
  `--launcher npx`: absolute path of `npx` + pinned `@agent-communications/gmail-mcp@<version>`.
...
- `.claude-plugin/marketplace.json` at the root (marketplace `agent-communications`, plugin `gmail`,
  ...
  `/usr/local/bin`, then execs the pinned `npx -y @agent-communications/gmail-mcp@X.Y.Z`). Validated in CI with
...
(`npx -y @agent-communications/gmail@<version> … --json`), because `npx skills add` installs skills without servers. Every skill: frontmatter (`name`, quoted `description`, `license: MIT`,
...
|---|---|---|---|
| **P1 Foundation + core** | `feat/foundation` | This spec and the plan; pnpm workspace, TypeScript 7, tsdown, Biome, CI (Linux, macOS, Windows); OSS files; `.blocks/`; ported and extended skill verifier; `@agent-communications/core` complete: paths, config (user intent only, locked writes, immutable inbox ids), runtime state, secret store (one backend, keychain with JS timeout and cache, file), audit log, envelope and exit codes, untrusted envelope **with taint recording**, HTML sanitiser, path jails, **approval engine with its full interface** (records, state machine, CAS transitions, digests, challenges, plan tokens), send ledger and rate caps, taint store, the `agentcomms` bin; a tarball-consumer test for `comms-core` so packaging is exercised from the first phase | — |
| **P2 Auth + lifecycle + surfaces** | `feat/gmail-auth` | `@agent-communications/gmail` skeleton; Gmail/People transport + retry; the local fake Google server used by tests; OAuth (loopback, detached two-step flow); client/inbox commands incl. reauth identity binding; import from artymclabin with the legacy-server scan; doctor; whoami; CLI skeleton (envelope, exit codes, coercion rules); MCP server skeleton on SDK v2 (handshake verified with Claude Code and Codex, `inboxes_list`, `whoami`, `doctor`); an elicitation round-trip test with the SDK v2 client; `mcp install`; `@agent-communications/gmail-mcp`; live check | P1 |
