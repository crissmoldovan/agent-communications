...
- **Architecture:** `@agentcomms/core` (provider-neutral: config, secrets, approvals, envelopes, jails) →
  `@agentcomms/gmail` (Gmail provider, operations, CLI, MCP server factory) → `@agentcomms/gmail-mcp` (the server as
...
- [x] `@agentcomms/gmail` package skeleton (tsdown: bundled `cli.mjs`, library `index.mjs`).
...
- [x] `mcp install --client …` (managed, npx and local launchers; proves the server starts),
      `@agentcomms/gmail-mcp`, packed-tarball consumer checks for both.
...
| `main-pins-unpublished-version` | packaging-oss | P2/P7 | Publish before the pins reach `main`. Tag and publish from the release branch, and fast-forward `main` only after `npm view @agentcomms/gmail-mcp@X.Y.Z version` succeeds … |
