...
The 27 tools the server offers, read from a running server.
...
| `text` | string | **yes** | what to say. Markup in it is shown, not interpreted |
| `threadTs` | string | no | reply inside this thread |
| `mentionUsers` | string[] | no | user ids to mention, by id — never by name |
| `broadcast` | `here` \\| `channel` \\| `everyone` | no | interrupts the room; posting it needs a person |
