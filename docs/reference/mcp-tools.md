...
The 38 tools the server offers, read from a running server.
...
| [`gmail_inbox_show`](#gmail_inbox_show) | Everything known about one mailbox: its address, tier and what it may do, how sending from it must be approved and whether that comes from the defaults, the OAuth client it signs in through, the scopes Google granted, its internal domains, and when it last refreshed. |
...
| [`gmail_inbox_policy`](#gmail_inbox_policy) | Set how sending from a mailbox must be approved: `chat` (the person says yes in this conversation), `confirm` (a code typed at a terminal, or into a trusted form) or `never` (not from here at all — the draft is sent from Gmail). |
...
|---|---|---|---|
| `inbox` | string | **yes** | which mailbox, by the name it was connected under (there is no default) |
| `sendPolicy` | `chat` \\| `confirm` \\| `never` | **yes** | chat, confirm or never |
