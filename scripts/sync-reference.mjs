...
/** Commands that only group others; their own help lists subcommands rather than doing anything. */
const GROUPS = new Set(['client', 'inbox', 'attachments', 'draft', 'send']);
...
async function commandPage(path) {
  const s = sections(await help(path));
  const lines = [`### \`agent-gmail ${path.join(' ')}\``, ''];
  if (s.description) lines.push(s.description, '');
  lines.push('```', s.usage.replace(/^agent-gmail\s*/, 'agent-gmail '), '```', '');
...
const cliParts = [
  ...
  table(
    commands.map(
      (c) =>
        `| [\`${cell(c.name.split(' ')[0])}\`](#agent-gmail-${c.name.split(' ')[0].split('|')[0]}) | ${cell(c.text)} |`,
    ),
...
for (const c of commands) {
  ...
  cliParts.push(await commandPage([name]), '');
  if (GROUPS.has(name)) {
    const sub = sections(await help([name]))
      .Commands.map(entry)
      .filter((x) => !/^help\b/.test(x.name));
    for (const s of sub) cliParts.push(await commandPage([name, s.name.split(' ')[0].split('|')[0]]), '');
  }
}
...
console.log(
  check
    ...
    : `reference written: ${commands.length} commands, ${all.length} MCP tools, ${skillDirs.length} skills.`,
);
