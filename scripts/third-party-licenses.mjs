...
async function bundledInto(name) {
  ...
  const direct = Object.keys({ ...manifest.dependencies, ...manifest.devDependencies })
    ...
    .filter((dependency) => !dependency.startsWith('@types/') && !dependency.startsWith('@cloudpixel/'))
...
for (const name of PACKAGES) {
  ...
  const content =
    notices.length === 0
      ? [
          `Third-party licences for @cloudpixel/${name}`,
          '',
          'This package inlines no third-party code: it depends on @cloudpixel/gmail at runtime, which carries its',
      ...
      : [
          `Third-party licences bundled into @cloudpixel/${name}`,
          '',
