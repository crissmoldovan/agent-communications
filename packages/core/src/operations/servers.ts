 ...
 *
 * `agentcomms mcp install` registers the core server; `comms_server_install` registers any of the three. Both are the
 * shared installer (`mcp-install.ts`) with a product built from `CHANNEL_SERVERS`, and both go through the change
 * flow: registering a server is a loosening in the design's terms (§3.1) — it hands a client a new set of tools — and
 * so is asked for once and applied on the second call with the approval. Printing an entry registers nothing and
 * asks nobody.
 *
 * The version installed is this core's own. The packages are released together, at one version, so the core that is
 * asked to install Gmail installs the Gmail of its own release.
 */
...
export function serverInstallChange(
  ...
  return {
    plan: async (config) => {
      ...
      const product = await channelProduct(request.channel, request.launcher);
    ...
    apply: async () => {
      const product = await channelProduct(request.channel, request.launcher);
      const result = await mcpInstall(context, product, installOptions(request));
      return {
        ...result,
...
export function serverPruneChange(
  ...
  const context: InstallContext = { env, core };
  const product = { packageName: CHANNEL_SERVERS[request.channel].packageName, version: VERSION };
  const label = CHANNEL_LABELS[request.channel];
  const base = {
