...
export function renderManifestHelp(mode: string, port: number, color: boolean): string {
  return [
    ...
    '',
    paint(color, 'dim', 'The Client ID is not a secret, and there is no client secret to copy: this signs in with'),
    paint(color, 'dim', 'PKCE, so nothing but the app’s public identifier ever leaves Slack’s page.'),
    '',
    'Then connect it:',
