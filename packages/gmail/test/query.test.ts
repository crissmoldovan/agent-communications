...
test('dates become absolute instants in the user’s timezone, not Gmail’s', () => {
  ...
  assert.deepEqual(london.rewrites, [
    { operator: 'after', from: '2026-09-17', to: String(Date.UTC(2026, 8, 16, 23) / 1000) },
  ]);
...
test('daylight saving is taken from the zone, not assumed', () => {
  ...
  assert.equal(
    localMidnightEpochSeconds({ year: 2026, month: 7, day: 1 }, 'Europe/London'),
    Date.UTC(2026, 5, 30, 23) / 1000,
  );
  assert.equal(
    localMidnightEpochSeconds({ year: 2026, month: 1, day: 1 }, 'Europe/London'),
    Date.UTC(2026, 0, 1, 0) / 1000,
  );
  // A zone with a half-hour offset.
  assert.equal(
    localMidnightEpochSeconds({ year: 2026, month: 3, day: 1 }, 'Asia/Kolkata'),
    Date.UTC(2026, 1, 28, 18, 30) / 1000,
  );
});
