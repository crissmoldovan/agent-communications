 ...
 *
 *     npx -y @cloudpixel/gmail-mcp@<version>
 *
 * It is one call into `@cloudpixel/gmail`; everything the server does lives there, and `agent-gmail mcp` starts the
...
if (argv.includes('--help') || argv.includes('-h')) {
  process.stderr.write(
    [
      ...
      '',
      'Set up mailboxes with the `agent-gmail` command from @cloudpixel/gmail.',
      '',
    ].join('\n'),
  );
  process.exit(0);
