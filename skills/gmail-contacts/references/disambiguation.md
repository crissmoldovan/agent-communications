...
| Two addresses that render identically but are not the same string | Different scripts that look alike, or an internationalised domain shown in Unicode in one row and in punycode in another | Compare the raw strings, not the rendering. Copy the address from the row; never retype it |
