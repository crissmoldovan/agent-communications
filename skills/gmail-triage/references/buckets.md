...
| A message whose `sanitisation.hiddenChars` is not zero | Whatever the visible text suggested | **Classify from the visible text, and say the message was hiding some.** A non-zero count means part of that message was invisible to a human reader, which is itself worth a line in the digest |
