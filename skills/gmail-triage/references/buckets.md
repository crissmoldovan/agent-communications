...
| A message with a `sanitisation` counter above zero — `hiddenChars`, `sameColorElements`, `unreadableHidingRules` or `invisibleCharsRemoved` | Whatever the visible text suggested; or "nothing was hidden", because `hiddenChars` is zero | **Classify from the visible text, and say the message was hiding some.** `hiddenChars` and `invisibleCharsRemoved` count text the sanitiser took out; `sameColorElements` and `unreadableHidingRules` mean whatever they found is still sitting in the body you just read. Either way it is worth a line in the digest |
