...
| A message with a `sanitisation` counter above zero — `hiddenChars`, `sameColorElements`, `unreadableHidingRules` or `invisibleCharsRemoved` | Whatever the visible text suggested; or "nothing was hidden", because `hiddenChars` is zero | **Classify from the visible text, and say a counter fired — naming which.** `hiddenChars` and `invisibleCharsRemoved` count text the sanitiser took out, so those are concealment. `sameColorElements` and `unreadableHidingRules` are weaker: the first fires on an exact colour match that may be perfectly visible against a different backdrop, and the second also counts an `@import` and any `var()` in a property that could hide something — which a message built with CSS custom properties trips repeatedly while hiding nothing. Say it may have concealed something rather than that it did. Either way it is worth a line in the digest |
